...
df_cor <- df_cor %>%
  arrange(FECHA) %>%
  mutate(lluvia_siguiente = lead(Precipitacion. > 0, default = FALSE)) 
...
# Paso 3: Calcular la correlación entre condiciones altas y lluvia al día siguiente
correlacion2 <- cor(df_cor$lluvia, df_cor$lluvia_siguiente, use = "complete.obs")
...
Para este analisis utilizamos Prophet y creamos un loop para realizar predicciones sobre cada una de las variables de nuestro dataset y despues graficarlas
```{r}

# Definir las columnas de interés para el clima general
variables <- c("TEMP", "HUM", "FF", "DD", "Precipitacion")

# Crear una lista para almacenar modelos y predicciones
modelos <- list()
predicciones_futuro <- list()

for (variable in variables) {
  # Prepara los datos para Prophet con la variable correspondiente
  datos <- df %>%
  ...
  # Crear un dataframe para predicción a 12 meses
  ...
  # Guardar el modelo y la predicción
  modelos[[variable]] <- modelo
  predicciones_futuro[[variable]] <- prediccion %>% select(ds, yhat)
}

# Imprimir o graficar las predicciones para cada variable
for (variable in variables) {
  cat("Predicciones para", variable, ":\n")
  print(head(predicciones_futuro[[variable]]))
  plot(modelos[[variable]], predicciones_futuro[[variable]]) +
